...
function getPizzaType(pizzaData) {
	return pizzaData[0];
}
...
function addTax(pizzaData) {
	const pizzaTotal = ((pizzaData[1]*0.0925)+ pizzaData[1]).toFixed(2);
	pizzaData.splice(2, 0, parseFloat(pizzaTotal)); 
...
function completeOrder(pizzaData) {
	pizzaData[3] = true;
...
function allTheNumbers(maxNumber) {
	numberArray = [];
